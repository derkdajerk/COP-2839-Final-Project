...
For Week 13, I focused on implementing diagnostics and health monitoring for the Workout Log Tracker application by adding a dedicated `/healthz` endpoint. The main goal was to provide a clear and reliable way to check the application's health status, while confirming its ability to connect to the SQL Server database, which is a critical dependency. To achieve this, I configured the health check middleware in `Program.cs` to expose the `/healthz` endpoint. I used the `.AddSqlServer(conStr)` method to ensure that the health check verifies database connectivity on every request. This means that whenever someone or an automated system accesses `/healthz`, they receive a real-time status report indicating whether the application and its database are operational.

To make the health check output useful for troubleshooting but still secure, I customized the response to show the overall status and considered best practices for including individual check results and error messages without exposing sensitive information like connection strings. I tested the endpoint by navigating to `/healthz` in the browser and confirmed that it returns the expected status output. I also updated the site navigation to include a direct link to the health check endpoint, making it easy to access from anywhere in the app. Overall, this addition improves the maintainability and reliability of the application by making it easier to detect and troubleshoot issues in real time.

## Week 14 - Logging

For Week 14, I focused on implementing structured logging throughout the Workout Log Tracker application to improve observability, troubleshooting, and maintainability. The main objective was to ensure that key actions and errors within the app are captured in a clear, actionable format, making it easier to monitor user activity and diagnose issues. I utilized the built-in `ILogger` service provided by ASP.NET Core, injecting it into my Razor Page models such as `CreateModel`, `DeleteModel`, and `EditModel` for the workout creation workflow. 

I made sure to log both successful and error paths. For example, when a new workout is created successfully, a log entry is written with the workout’s name, ID, and a timestamp, providing a clear audit trail of user actions. If an error occurs during workout creation an error log is generated with the same contextual details, making it straightforward to identify and resolve problems.

To further enhance the usefulness of the logs, I included the request ID from `HttpContext.TraceIdentifier`, which would allow for correlation across multiple log entries and requests. This approach ensures that logs are not only informative but also actionable for both developers and system administrators. I verified the logging implementation by intentionally triggering validation errors and observing the resulting log entries in the console output. Screenshots of these logs have been included in the repository as evidence.

## Week 15 - Stored Procedures

For Week 15, I focused on implementing stored procedures for the Workout Log Tracker application. The main objective was to create a dedicated stored procedure that summarizes workout data by muscle group, allowing users to gain insights into their training patterns and consistency. I designed and created the `sp_GetWorkoutSummary` stored procedure in SQL Server, which groups workouts by muscle group and calculates key metrics including the total workout count per muscle group and the most recent workout date for each group.
